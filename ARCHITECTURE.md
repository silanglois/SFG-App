...
- `src/sfg_app2/processing/` — pure Python processing pipeline (no Qt imports)
  ...
  - `image_file.py` — CCD image-data file loading
...
- `src/sfg_app2/app/` — PySide6 GUI application
  ...
  - `utils/` — settings persistence (dock layouts, plotting style, matching profiles, color coding, metadata patterns, fit templates), color coding, phase wrapping
...
- Settings persistence (dock layouts, plotting style, matching profiles, color coding, metadata patterns, fit templates) each independently follow the same load/save-to-JSON-via-`platformdirs` pattern (`app/utils/*_settings.py`, `pattern_manager.py`, `fit_template_manager.py`)
